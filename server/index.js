...
//middlewares - functions that have access to the req and res obj and can perform task specified. - execute a piece of code, make changes to the req or res obj, call the next handler function. It basically helps to add and reuse functions across the approutes and endpoints. The flow -
...
//5, response is finally sent to the clients

app.use(
  cors({
    origin: [
      "http://localhost:4800",
      "https://clinicare-clientcode.vercel.app",
...
app.use(express.json({ limit: "25mb" })); //passes request gotten from client side in a body no greater than 25mb
...
const connectDb = async () => {
  const connectionOptions = {
    ...
    dbName: process.env.MONGODB_DB_NAME, //read env files.
    serverSelectionTimeoutMs: 45000, //max time to wait for a server to be selected (45 secs in ours), if no server selecction a timeout error is thrown.
    socketTimeoutMs: 5000, //timeouts before socket timeout due to inactivity, useful to avoid hanging connections
    ...
    maxPoolSize: 50, //maximum number of connections in the mongodb connection pool, this helps to manage concurent request.
  ...
  try {
    ...
    mongoose.connection.on("disconnected", () =>
      console.log("ℹ️ Mongodb Connected")
    );
    ...
    process.on("SIGNI", gracefulShutdown); //ctrl + C
    process.on("SIGTERM", gracefulShutdown); //a signal to terminate a process
    return conn;
  } catch (error) {
    console.error("❌ Mongodb connection failed", error.message);
    process.exit(1); //exit the process, I usually indicate error/failure
  }
};
...
process.on("uncaughtException", (err) => {
  ...
  concole.error(err.name, err.message);
  process.exit(1);
});
