...
export const register = tryCatchFn(async (req, res, next) => {
  ...
  if (!user) return;
  const { accessToken, refreshToken, cookieOption } = createSendToken(user);
  //send the cookie..
  res.cookie("userRefreshToken", refreshToken, cookieOption);
  return successResponse(res, { accessToken }, "Registeration successful", 201);
});
...
export const authenticateUser = tryCatchFn(async (req, res, next) => {
  const { id: userId } = req.user; //extracts user Id from the request.user
...
export const forgotPassword = tryCatchFn(async (req, res, next) => {
  ...
  return successResponse(
    res,
    null,
    "Password reset link has been sent to yor email",
    200
  );
});
