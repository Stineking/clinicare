...
export const cache = new NodeCache({
  ...
  useClones: false, //better performance for caching systems
});
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      res.json = function (data) {
        ...
        return originalJSON.call(this, data); //originslJson is the res.json first saved, the call is used to invoke the method.so that the original res.json is seen with the help of this keyword, ensuring the data can be properly passed to the original Json
      };
      next(); //call the next event or action supposed to happen.
    } catch (error) {
